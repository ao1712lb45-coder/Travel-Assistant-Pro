...
function getDb(config){
  ...
  cached=firestoreSdk.getFirestore(app);cached.settings({ignoreUndefinedProperties:true});return cached;
}
...
async function addSnapshot(config,type,data){const db=getDb(config),row={type,count:data.length,created_at:new Date().toISOString(),chunked:true},ref=await db.collection('travel_backups').add(row);for(let start=0,index=0;start<data.length;start+=25,index++){await ref.collection('chunks').doc(String(index).padStart(6,'0')).set({data:data.slice(start,start+25)})}return{id:ref.id,...row}}
...
async function snapshot(config,id,type){const ref=getDb(config).collection('travel_backups').doc(String(id||'')),doc=await ref.get(),row=doc.exists?doc.data():null;if(!row||row.type!==type)throw new CloudStoreError('SNAPSHOT_NOT_FOUND','找不到指定的雲端備份。',404);if(Array.isArray(row.data))return row.data;const chunks=await ref.collection('chunks').get();return chunks.docs.sort((a,b)=>a.id.localeCompare(b.id)).flatMap(item=>Array.isArray(item.data().data)?item.data().data:[])}
