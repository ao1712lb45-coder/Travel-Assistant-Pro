...
(function (global) {
  ...
  function drawImageCover(ctx, image, x, y, w, h, offsetPercent = 0) {
    const scale = Math.max(w / image.width, h / image.height);
    const sw = w / scale, sh = h / scale;
    const availableY = Math.max(0, image.height - sh);
    const sourceY = Math.max(0, Math.min(availableY, availableY / 2 + availableY * offsetPercent / 80));
    ctx.drawImage(image, (image.width - sw) / 2, sourceY, sw, sh, x, y, w, h);
  }
  function drawPhoto(ctx, image, x, y, w, h, radius = 22, offsetPercent = 0, darkness = 0) {
    ctx.save(); roundPath(ctx, x, y, w, h, radius); ctx.clip();
    if (image) drawImageCover(ctx, image, x, y, w, h, offsetPercent);
    else {
    ...
    if (image && darkness > 0) { ctx.fillStyle = `rgba(0,0,0,${Math.min(.75, darkness / 100)})`; ctx.fillRect(x, y, w, h); }
  ...
  function collage(ctx, w, h) {
    ...
    const offset = Number($('photoY').value) || 0, darkness = Number($('photoDark').value) || 0;
    if ($('dmLayout').value === 'hero') { drawPhoto(ctx, photos[0], x, y, areaW, areaH, 28, offset, darkness); return; }
    const cellW=(areaW-gap)/2, cellH=(areaH-gap)/2;
    drawPhoto(ctx,photos[0],x,y,cellW,cellH,22,offset,darkness); drawPhoto(ctx,photos[1],x+cellW+gap,y,cellW,cellH,22,offset,darkness);
    drawPhoto(ctx,photos[2],x,y+cellH+gap,cellW,cellH,22,offset,darkness); drawPhoto(ctx,photos[3]||photos[0],x+cellW+gap,y+cellH+gap,cellW,cellH,22,offset,darkness);
  }
  ...
  function draw() {
    ...
    const titleScale=(Number($('titleScale').value)||100)/100,priceScale=(Number($('priceScale').value)||100)/100;
    const titleWidth=w*.44, titleSize=fitText(ctx,d.title,titleWidth,Math.round(88*titleScale),Math.round(42*titleScale),900,font);ctx.font=`900 ${titleSize}px ${font}`;
    ...
    ctx.fillStyle='#d71920';const amount=priceNumber(d.price);ctx.font=`900 ${Math.round(86*priceScale)}px Arial`;ctx.textAlign='right';ctx.fillText(amount?amount.toLocaleString('en-US'):d.price,w*.93,cardY+220);ctx.font=`800 ${Math.round(24*priceScale)}px ${font}`;ctx.fillText(amount?'元起':'',w*.93,cardY+302);ctx.textAlign='left';
  ...
  // Replace legacy event handlers so one control can never render both old and new DM engines.
  $('dmLayout').onchange=draw;
  $('dmFont').onchange=draw;
  ['dmHeadline','dmSlogan'].forEach(id=>$(id).addEventListener('input',draw));
  ['size','theme','highlightCount'].forEach(id=>$(id).onchange=draw);
  ['titleScale','priceScale','photoY','photoDark'].forEach(id=>{
    $(id).oninput=()=>{ const suffix=(id==='photoY'&&Number($(id).value)>0?'+':'')+$(id).value+'%'; $(id+'Val').textContent=suffix; draw(); };
  });
