...
(function (global) {
  ...
  function fitText(ctx, text, maxWidth, startSize, minSize, weight = 900) {
    let size = startSize;
    do { ctx.font = `${weight} ${size}px Microsoft JhengHei, sans-serif`; size -= 2; }
  ...
  global.TravelDmDesigner = { splitDates, priceNumber };
  if (typeof module !== 'undefined') module.exports = { splitDates, priceNumber };
  ...
  function values() {
    ...
    return { title:$('mainTitle').value.trim() || '精選旅遊行程', subtitle:$('subtitle').value.trim(), code:$('code').value.trim(),
  ...
  function pill(ctx, text, x, y, color) {
    ctx.font='800 25px Microsoft JhengHei'; const width=Math.min(230,ctx.measureText(text).width+38);
  ...
  function draw() {
    const [w,h]=$('size').value.split('x').map(Number),ctx=canvas.getContext('2d'),d=values(),portrait=h/w>1.5;
    ...
    const titleWidth=w*.44, titleSize=fitText(ctx,d.title,titleWidth,88,48);ctx.font=`900 ${titleSize}px Microsoft JhengHei`;
    ...
    ctx.strokeStyle='#fff';ctx.lineWidth=8;ctx.fillStyle='#f23867';ctx.font=`900 ${Math.round(titleSize*.78)}px Microsoft JhengHei`;
    ...
    ctx.fillStyle='#172033';ctx.font='900 39px Microsoft JhengHei';ctx.fillText(d.title,w*.06,cardY+34);
    ctx.font='700 25px Microsoft JhengHei';ctx.fillStyle='#344054';ctx.fillText(`✈ ${d.airline}　${d.code}`,w*.06,cardY+88);
    let px=w*.06,py=cardY+136;(d.highlights.length?d.highlights:['精選景點','特色餐食','安心服務']).slice(0,4).forEach((item,i)=>{const colors=['#e83d64','#f28b20','#1686c9','#55a630'];px+=pill(ctx,item.slice(0,8),px,py,colors[i%4])+12;if(px>w*.82){px=w*.06;py+=60;}});
    ctx.fillStyle='#d71920';const amount=priceNumber(d.price);ctx.font='900 86px Arial';ctx.textAlign='right';ctx.fillText(amount?amount.toLocaleString('en-US'):d.price,w*.93,cardY+220);ctx.font='800 24px Microsoft JhengHei';ctx.fillText(amount?'元起':'',w*.93,cardY+302);ctx.textAlign='left';
    const dates=d.dates.length?d.dates:['日期待確認'];let dx=w*.06,dy=cardY+225;ctx.font='900 26px Microsoft JhengHei';ctx.fillStyle='#fff';dates.slice(0,6).forEach((date,i)=>{const label=date.replace(/^20\d{2}[\/-]/,'').replace(/[\/-]/g,'/');ctx.fillStyle=['#ec476c','#ef8b22','#1686c9','#62a839','#8f62bc'][i%5];roundPath(ctx,dx,dy,125,56,14);ctx.fill();ctx.fillStyle='#fff';ctx.textAlign='center';ctx.fillText(label,dx+62,dy+13);dx+=137;if(dx>w*.54){dx=w*.06;dy+=66;}});ctx.textAlign='left';
    ctx.fillStyle='#0c6b52';ctx.fillRect(0,h-h*.075,w,h*.075);ctx.fillStyle='#fff';ctx.font='900 31px Microsoft JhengHei';ctx.fillText(d.contact||'喜鴻假期',w*.055,h-h*.057);ctx.textAlign='right';ctx.font='900 37px Arial';ctx.fillText('LINE '+(d.line||''),w*.945,h-h*.061);ctx.textAlign='left';
  ...
  makeButton.onclick=()=>{ if(typeof global.generateCopy==='function') global.generateCopy(); draw(); };
