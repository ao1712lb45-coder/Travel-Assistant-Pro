(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.TravelCloudDatabase=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function install(){
    ...
    section.querySelector('h2').insertAdjacentElement('afterend',panel);panel.querySelector('b').textContent='雲端資料庫（優先使用 Firebase Firestore）';
    ...
    async function syncNow(refreshCloud=true){
      if(busy)return;busy=true;progress(5,'正在連接雲端資料庫');byId('cloudDatabaseStatus').textContent='正在同步雲端資料庫…';
      try{
    ...
    byId('cloudSyncNow').onclick=()=>syncNow(true);document.addEventListener('travel:database-updated',()=>{const local=readLocal(),pending=changedTrips(local,known);details(local.length,Math.max(0,local.length-pending.length),pending.length,localStorage.getItem('travelCloudLastSync'));if(configured&&pending.length)syncNow(false)});
    ...
    details(readLocal().length,0,0,localStorage.getItem('travelCloudLastSync'));loadSnapshots();syncNow(true);
  }
