...
(function () {
  ...
  style.textContent = `
    ...
    header{padding:10px clamp(16px,3vw,34px);background:linear-gradient(125deg,#073f43 0%,#007f78 58%,#e9863b 140%);box-shadow:0 6px 18px rgba(4,50,53,.18)}
    header h1{font-size:clamp(19px,2.3vw,24px);letter-spacing:.02em;margin-bottom:2px}header p{max-width:760px;line-height:1.35;font-size:12px}
    .v2-kicker{display:inline-flex;align-items:center;gap:5px;margin-bottom:4px;padding:3px 8px;border:1px solid rgba(255,255,255,.32);border-radius:999px;background:rgba(255,255,255,.12);font-size:9px;font-weight:800;letter-spacing:.07em}
    ...
    .hint{background:#f5faf9;border-color:#dbeae9}.badge{background:#e3f5f2;color:#087069}.output{min-height:300px;background:#eef1f3;border-color:#b8c7ca;line-height:1.65;caret-color:#075e59}.output:focus{background:#e8edef}
  ...
  document.title = 'Travel Assistant Pro 2.2';
  const header = document.querySelector('header');
  if (header) header.innerHTML = '<div class="v2-kicker">TRAVEL ASSISTANT PRO · 2.2</div><h1>旅行社智慧業務工作台</h1><p>官網行程擷取、完整內容搜尋、行銷素材、雲端 CRM 與安全備份，一個網站完成日常工作。</p>';
})();
