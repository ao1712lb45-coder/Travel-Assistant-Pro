...
(function(global,factory){const api=factory(global);if(typeof module==='object'&&module.exports)module.exports=api;global.TravelSearchAssistant=api;})(typeof window!=='undefined'?window:globalThis,function(global){
  ...
  function ittmsUrl(code){const value=clean(code).toUpperCase();return value?`https://itinerary.ittms.com.tw/?travel_no=${encodeURIComponent(value)}&agt_no=${ITTMS_AGENT}`:''}
  ...
  function moneyValue(value){const source=clean(value).replace(/[,，\s]/g,''),match=source.match(/(\d+(?:\.\d+)?)\s*(萬|千)?/);if(!match)return 0;const multiplier=match[2]==='萬'?10000:match[2]==='千'?1000:1;return Math.round(Number(match[1])*multiplier)}
  function priceRange(source){
    const token='(\\d+(?:\\.\\d+)?\\s*(?:萬|千|元)?)';
    let match=source.match(new RegExp(`${token}\\s*(?:-|~|～|至|到)\\s*${token}`));
    if(match){const a=moneyValue(match[1]),b=moneyValue(match[2]),looksLikeMoney=/萬|千|元/.test(match[0])||a>=1000||b>=1000;if(looksLikeMoney&&!/^[^萬千元]*[月日]/.test(source.slice(match.index+match[0].length)))return {raw:match[0],minPrice:Math.min(a,b),maxPrice:Math.max(a,b)}}
    match=source.match(new RegExp(`${token}\\s*(?:元)?\\s*(?:以內|以下|內|不超過|最高|上限)`));
    if(match)return {raw:match[0],minPrice:0,maxPrice:moneyValue(match[1])};
    match=source.match(new RegExp(`${token}\\s*(?:元)?\\s*(?:以上|起|最低|至少)`));
    if(match)return {raw:match[0],minPrice:moneyValue(match[1]),maxPrice:0};
    match=source.match(new RegExp(`(?:預算|價格|價位)\\s*${token}`));
    if(match)return {raw:match[0],minPrice:0,maxPrice:moneyValue(match[1])};
    return {raw:'',minPrice:0,maxPrice:0};
  }
  ...
  function parseSearchRequest(text,now=new Date()){
    ...
    const budget=priceRange(source);
  ...
  function install(){
    ...
    const section=document.createElement('section');section.id='searchAssistantSection';section.className='section';section.innerHTML=`<h2>行程搜尋助手 <span class="badge">免付費 AI</span></h2><div class="hint">直接輸入月份、景點與價格，例如「明年 1 月北海道 5 萬內」或「日本 3 萬到 6 萬」。會搜尋目前瀏覽器的行程資料庫與已保存官網全文。</div><label style="margin-top:12px">想找什麼行程？</label><textarea id="assistantSearchInput" placeholder="例如：明年 1 月北海道 5 萬內" style="min-height:90px"></textarea><div class="grid2" style="margin-top:10px"><div><label>最低價格（選填）</label><input id="assistantMinPrice" type="number" min="0" step="1000" placeholder="例如：30000"></div><div><label>最高價格（選填）</label><input id="assistantMaxPrice" type="number" min="0" step="1000" placeholder="例如：60000"></div></div><div class="small" style="margin-top:5px">可直接寫在搜尋文字裡；下方金額欄位若有填寫，會以欄位為準。</div><div class="btnrow"><button class="primary" id="assistantSearchButton">搜尋行程</button><button id="assistantSelectAll">全選結果</button><button id="assistantClearSelection">取消全選</button></div><div id="assistantSearchStatus" class="status"></div><div id="assistantSearchResults" style="display:grid;gap:8px;margin-top:12px"></div><div class="btnrow"><button class="dark" id="assistantGenerateCopy">依勾選行程產生文案</button><button id="assistantSendToMarketing">送到行銷素材</button><button id="assistantCopyText">複製文案</button></div><textarea id="assistantCopyOutput" class="output" style="margin-top:10px;min-height:320px" placeholder="勾選行程後產生 LINE 社群文案"></textarea>`;panel.appendChild(section);
    ...
    byId('assistantSearchButton').onclick=async()=>{const button=byId('assistantSearchButton'),request=parseSearchRequest(byId('assistantSearchInput').value),fieldMin=Number(byId('assistantMinPrice').value)||0,fieldMax=Number(byId('assistantMaxPrice').value)||0;if(fieldMin)request.minPrice=fieldMin;if(fieldMax)request.maxPrice=fieldMax;if(request.minPrice&&request.maxPrice&&request.minPrice>request.maxPrice){status('最低價格不能高於最高價格。','warn');return}if(!request.keyword&&!request.months.length&&!request.year&&!request.minPrice&&!request.maxPrice){status('請輸入月份、年份、景點或價格條件。','warn');return}button.disabled=true;let supplemented=0;try{results=searchTrips(database(),request);if(!results.length&&request.keyword){status('共用資料庫沒有符合行程，正在補查 Besttour 官網…','warn');supplemented=(await fetchOfficial(request)).length;results=searchTrips(database(),request)}render();status(`${searchSummary(request,results)}${supplemented?`｜官網補入 ${supplemented} 團`:''}`,results.length?'ok':'warn')}catch(error){status(`官網補查失敗：${error.message}，請稍後再試。`,'warn')}finally{button.disabled=false}};
  ...
  return {parseSearchRequest,tripDates,productKey,ittmsUrl,mergeProducts,searchTrips,officialSearchPlan,searchSummary,install};
});
