...
(function(global,factory){const api=factory(global);if(typeof module==='object'&&module.exports)module.exports=api;global.TravelSearchAssistant=api;})(typeof window!=='undefined'?window:globalThis,function(global){
  ...
  function parseSearchRequest(text,now=new Date()){
    ...
    return {raw:source,year:resolvedYear,month,months,period,dayRange,dateRange,holiday:holiday&&holiday.name||'',keyword};
  }
  ...
  function install(){
    ...
    async function fetchOfficial(request){const plan=officialSearchPlan(request),found=[];for(const keyword of plan.keywords){const params=new URLSearchParams({keyword,page:'1',pageSize:'50'});if(plan.dateFrom)params.set('dateFrom',plan.dateFrom.replaceAll('-','/'));if(plan.dateTo)params.set('dateTo',plan.dateTo.replaceAll('-','/'));const response=await fetch('/api/besttour/search?'+params);const payload=await response.json();if(response.ok&&payload.ok)found.push(...(payload.data.trips||[]))}const unique=[...new Map(found.map(trip=>[clean(trip.code).toUpperCase(),trip])).values()];if(unique.length)saveDatabase(unique);return unique}
