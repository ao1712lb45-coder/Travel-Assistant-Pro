...
test('eight variants produce different copy sets',()=>{const out=Array.from({length:8},(_,i)=>generateSet(trip,'natural',i+1));assert.equal(new Set(out.map(x=>x.line)).size,8);assert.equal(new Set(out.map(x=>x.facebook)).size,8);assert.equal(new Set(out.map(x=>x.threads)).size,8)});
