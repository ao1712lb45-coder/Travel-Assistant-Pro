...
test('builds one enterprise email from multiple selected trips',()=>{
  ...
  assert.match(result.subject,/範例科技/);assert.match(result.body,/王小姐 您好/);assert.match(result.body,/以下 2 個/);assert.match(result.body,/九州五日/);assert.match(result.body,/釜山五日/);assert.match(result.body,/釜山航空/);assert.match(result.body,/0988894313/);
});
